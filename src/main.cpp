...
#if defined(NDEBUG)
#error "Fortcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Fortcoin Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // 4.2 million for ICO, 60,000 for 6 masternodes until the network is established, the remainder
    // for staking wallets to run the network until it's established.
    if (nHeight == 1) {
        nSubsidy = 4280010 * COIN;
    }
...
{
    RenameThread("fortcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!IsInitialBlockDownload() && !IsBlockValueValid(block, GetBlockValue(pindex->pprev->nHeight + 1))) {
        return state.DoS(100,
            error("ConnectBlock() : reward pays too much (actual=%d vs limit=%d)",
                block.vtx[0].GetValueOut(), GetBlockValue(pindex->pprev->nHeight + 1)),
...
{
    // Check proof of work matches claimed amount on POW Blocks
...
{
    // DISABLE POW Checks on Stake Blocks
    if (!block.IsProofOfWork())
        fCheckPOW = false;

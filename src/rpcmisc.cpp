...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total fortcoin balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized fortcoin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in fortcoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in fortcoin/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"fortcoinaddress\"\n"
            "\nReturn information about the given fortcoin address.\n"
            "\nArguments:\n"
            "1. \"fortcoinaddress\"     (string, required) The fortcoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"fortcoinaddress\", (string) The fortcoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Fortcoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are fortcoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) fortcoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"fortcoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"fortcoinaddress\"  (string, required) The fortcoin address to use for the signature.\n"
